import { TerrainType, BeeRole, BeeState } from '../types';
import { hexToPixel, hexRing, hexDisk, hexDistance } from '../hex/hex';
import { WORLD_RADIUS, FLOWER_CLUSTER_COUNT, FLOWER_MIN_DISTANCE, FLOWER_MAX_DISTANCE, INITIAL_NECTAR_MIN, INITIAL_NECTAR_MAX, STARTING_FORAGERS, STARTING_NURSES, STARTING_BUILDERS, } from '../constants';
...
function placeFlowers(world) {
    ...
    for (let i = 0; i < FLOWER_CLUSTER_COUNT; i++) {
        ...
        while (attempts < 100) {
            ...
            if (hexDistance(0, 0, fq, fr) < FLOWER_MIN_DISTANCE)
                continue;
            if (hexDistance(0, 0, fq, fr) > FLOWER_MAX_DISTANCE)
                continue;
            let tooClose = false;
            for (const p of placed) {
                if (hexDistance(p.q, p.r, fq, fr) < 3) {
                    tooClose = true;
                    break;
                }
            }
...
function spawnStartingBees(world) {
    const roles = [
...
export function createBee(world, role, q, r) {
