import { BeeRole, BeeState, TerrainType } from '../types';
import { hexDistance, hexNeighbors } from '../hex/hex';
import { HARVEST_TICKS, DEPOSIT_TICKS, NECTAR_PER_HARVEST, BEE_CARRY_CAPACITY, SCORE_DISTANCE_WEIGHT, SCORE_PHEROMONE_WEIGHT, SCORE_NECTAR_WEIGHT, SCORE_JITTER, TEND_TICKS, BROOD_TEND_AMOUNT, BROOD_HONEY_COST, PROCESS_TICKS, PROCESSING_RATE, NECTAR_TO_HONEY_RATIO, NIGHT_START, DAWN_END, } from '../constants';
export function updateBeeAI(bee, world) {
...
function updateForager(bee, world) {
    switch (bee.state) {
        case BeeState.Idle: {
...
function updateNurse(bee, world) {
    switch (bee.state) {
        case BeeState.Idle: {
...
function updateBuilder(bee, world) {
...
function findBroodNeedingCare(world) {
    const broods = world.grid.cellsOfType(TerrainType.Brood);
    for (const b of broods) {
        if (b.broodActive && b.broodProgress < 1)
            return b;
    }
    return null;
}
function findProcessingCell(world) {
    const cells = world.grid.cellsOfType(TerrainType.Processing);
    for (const c of cells) {
        if (c.nectarStored > 0.01)
            return c;
    }
    return null;
}
function findEmptyBrood(world) {
    const broods = world.grid.cellsOfType(TerrainType.Brood);
    for (const b of broods) {
        if (!b.broodActive)
            return b;
...
export function computePath(fromQ, fromR, toQ, toR, _world) {
    const path = [];
    let cq = fromQ;
    let cr = fromR;
