import { BeeEntity, BeeRole, BeeState, TerrainType, HexCell } from '../types';
import { World } from '../world/world';
import { hexDistance, hexNeighbors, hexKey } from '../hex/hex';
import { ensureChunksAroundPoint } from '../world/procgen';
import {
  DEPOSIT_TICKS, NECTAR_PER_HARVEST, POLLEN_PER_HARVEST, BEE_CARRY_CAPACITY, FLOWER_TYPE_CONFIG,
  SCORE_DISTANCE_WEIGHT, SCORE_NECTAR_WEIGHT, SCORE_POLLEN_WEIGHT, SCORE_JITTER, SCORE_WAYSTATION_WEIGHT, SCORE_WAYSTATION_MAX_DIST,
  TEND_TICKS, BROOD_TEND_AMOUNT, BROOD_HONEY_COST, BROOD_BEE_BREAD_COST, PROCESS_TICKS, PROCESSING_RATE,
  NECTAR_TO_HONEY_RATIO, NECTAR_STORAGE_CAPACITY,
  SCOUT_EXPLORE_RANGE, SCOUT_WAYSTATION_EXPLORE_RANGE, WAGGLE_DANCE_TICKS, POLLEN_STORAGE_CAPACITY,
  ENERGY_HUNGER_THRESHOLD, ENERGY_RESTORE_PER_TICK, HONEY_PER_EAT_TICK, EAT_TICKS,
  WAYSTATION_NECTAR_CAPACITY, WAYSTATION_POLLEN_CAPACITY,
  HAULER_CARRY_CAPACITY, HAULER_PICKUP_TICKS,
  BUILDER_WAX_WORK_TICKS, WAX_WORKS_BUILDER_RATE, WAX_WORKS_HONEY_TO_WAX_RATIO,
  NECTAR_CELL_CAPACITY,
} from '../constants';

export function updateBeeAI(bee: BeeEntity, world: World): void {
  // Exhaustion interrupt (highest priority) — bees at 0 energy must rest
  if (bee.energy <= 0 && bee.state !== BeeState.Resting && bee.state !== BeeState.Eating) {
    // Drop any carried resources where they stand
    bee.carrying.nectar = 0;
    bee.carrying.pollen = 0;
    if (bee.q === 0 && bee.r === 0) {
      bee.state = BeeState.Resting;
      bee.path = [];
    } else {
      bee.state = BeeState.Resting;
      bee.path = computePath(bee.q, bee.r, 0, 0, world);
    }
    return;
  }

  // Resting state — stay at hive until energy recovers above hunger threshold
  if (bee.state === BeeState.Resting) {
    if (bee.path.length === 0 && bee.energy >= ENERGY_HUNGER_THRESHOLD) {
      bee.state = BeeState.Idle;
    }
    return;
  }

  // Hunger interrupt (highest priority after exhaustion) — all roles
  if (bee.state !== BeeState.Hungry && bee.state !== BeeState.Eating) {
    if (bee.energy < ENERGY_HUNGER_THRESHOLD) {
      const honeyCell = findHoneyCellWithHoney(world, bee.q, bee.r);
      if (honeyCell) {
        bee.state = BeeState.Hungry;
        bee.targetQ = honeyCell.q;
        bee.targetR = honeyCell.r;
        bee.path = computePath(bee.q, bee.r, honeyCell.q, honeyCell.r, world);
        return;
      }
    }
  }

  // Handle hunger/eating states for any role
  if (bee.state === BeeState.Hungry) {
    if (bee.path.length === 0) {
      const cell = world.grid.get(bee.q, bee.r);
      if (cell && cell.terrain === TerrainType.HoneyStorage && cell.honeyStored > 0.01) {
        bee.state = BeeState.Eating;
        bee.stateTimer = EAT_TICKS;
      } else {
        // Couldn't reach honey, try again
        bee.state = BeeState.Idle;
      }
    }
    return;
  }

  if (bee.state === BeeState.Eating) {
    bee.stateTimer--;
    const cell = world.grid.get(bee.q, bee.r);
    if (cell && cell.honeyStored > 0) {
      const consumed = Math.min(HONEY_PER_EAT_TICK, cell.honeyStored);
      cell.honeyStored -= consumed;
      world.resources.honey -= consumed;
      bee.energy = Math.min(1, bee.energy + ENERGY_RESTORE_PER_TICK);
    }
    if (bee.stateTimer <= 0 || bee.energy >= 0.95) {
      bee.state = BeeState.Idle;
    }
    return;
  }

  switch (bee.role) {
    case BeeRole.Forager:
      updateForager(bee, world);
      break;
    case BeeRole.Nurse:
      updateNurse(bee, world);
      break;
    case BeeRole.Scout:
      updateScout(bee, world);
      break;
    case BeeRole.Hauler:
      updateHauler(bee, world);
      break;
    case BeeRole.Builder:
      updateBuilder(bee, world);
      break;
  }
}

/** Shared deposit logic: deposit nectar/pollen at the hive */
function depositAtHive(bee: BeeEntity, world: World): void {
  const wsAdjacentKeys = world.grid.waystationAdjacentStorageKeys();

  // Deposit nectar to processing cells first, then overflow to nectar storage
  if (bee.carrying.nectar > 0) {
    const processingCells = world.grid.cellsOfType(TerrainType.Processing);
    for (const cell of processingCells) {
      if (bee.carrying.nectar <= 0) break;
      const space = 3 - cell.nectarStored;
      if (space > 0) {
        const amount = Math.min(bee.carrying.nectar, space);
        cell.nectarStored += amount;
        world.resources.nectar += amount;
        bee.carrying.nectar -= amount;
      }
    }
    // Overflow into hive nectar storage cells (not waystation-adjacent ones)
    if (bee.carrying.nectar > 0) {
      const nectarStorageCells = world.grid.cellsOfType(TerrainType.NectarStorage)
        .filter(c => !wsAdjacentKeys.has(hexKey(c.q, c.r)));
      for (const cell of nectarStorageCells) {
        if (bee.carrying.nectar <= 0) break;
        const space = NECTAR_CELL_CAPACITY - cell.nectarStored;
        if (space > 0) {
          const amount = Math.min(bee.carrying.nectar, space);
          cell.nectarStored += amount;
          world.resources.nectar += amount;
          bee.carrying.nectar -= amount;
        }
      }
    }
    bee.carrying.nectar = 0;
  }

  // Deposit pollen to hive pollen storage cells (not waystation-adjacent ones)
  if (bee.carrying.pollen > 0) {
    const pollenCells = world.grid.cellsOfType(TerrainType.PollenStorage)
      .filter(c => !wsAdjacentKeys.has(hexKey(c.q, c.r)));
    for (const cell of pollenCells) {
      const space = POLLEN_STORAGE_CAPACITY - cell.pollenStored;
      if (space > 0) {
        const amount = Math.min(bee.carrying.pollen, space);
        cell.pollenStored += amount;
        bee.carrying.pollen -= amount;
        break;
      }
    }
    bee.carrying.pollen = 0;
  }
}

/** Calculate effective capacity of a waystation including adjacent storage cells */
function waystationEffectiveSpace(ws: HexCell, world: World): { nectar: number; pollen: number } {
  let nectarSpace = WAYSTATION_NECTAR_CAPACITY - ws.nectarStored;
  let pollenSpace = WAYSTATION_POLLEN_CAPACITY - ws.pollenStored;

  const neighbors = hexNeighbors(ws.q, ws.r);
  for (const nb of neighbors) {
    const cell = world.grid.get(nb.q, nb.r);
    if (!cell) continue;
    if (cell.terrain === TerrainType.NectarStorage) {
      nectarSpace += NECTAR_CELL_CAPACITY - cell.nectarStored;
    } else if (cell.terrain === TerrainType.Processing) {
      nectarSpace += NECTAR_STORAGE_CAPACITY - cell.nectarStored;
    } else if (cell.terrain === TerrainType.PollenStorage) {
      pollenSpace += POLLEN_STORAGE_CAPACITY - cell.pollenStored;
    }
  }

  return { nectar: nectarSpace, pollen: pollenSpace };
}

/** Find the best deposit target for a forager: nearest waystation with capacity or hive */
function findDepositTarget(bee: BeeEntity, world: World): { q: number; r: number } {
  const hiveDist = hexDistance(bee.q, bee.r, 0, 0);
  let bestDist = hiveDist;
  let bestTarget = { q: 0, r: 0 };

  const waystations = world.grid.waystationCells();
  for (const ws of waystations) {
    const space = waystationEffectiveSpace(ws, world);
    if (space.nectar < 0.01 && space.pollen < 0.01) continue;
    const dist = hexDistance(bee.q, bee.r, ws.q, ws.r);
    if (dist < bestDist) {
      bestDist = dist;
      bestTarget = { q: ws.q, r: ws.r };
    }
  }

  return bestTarget;
}

function updateForager(bee: BeeEntity, world: World): void {
  switch (bee.state) {
    case BeeState.Idle: {
      const flower = findBestFlower(bee, world);
      if (flower) {
        bee.targetQ = flower.q;
        bee.targetR = flower.r;
        bee.state = BeeState.FlyingToFlower;
        bee.path = computePath(bee.q, bee.r, flower.q, flower.r, world);
      }
      break;
    }

    case BeeState.FlyingToFlower: {
      if (bee.path.length === 0) {
        const cell = world.grid.get(bee.q, bee.r);
        if (cell && cell.terrain === TerrainType.Flower && cell.nectarAmount > 0.01) {
          bee.state = BeeState.Harvesting;
          bee.stateTimer = FLOWER_TYPE_CONFIG[cell.flowerType].harvestTicks;
        } else {
          bee.state = BeeState.Idle;
        }
      }
      break;
    }

    case BeeState.Harvesting: {
      bee.stateTimer--;
      if (bee.stateTimer <= 0) {
        const cell = world.grid.get(bee.q, bee.r);
        if (cell) {
          const nectarHarvest = Math.min(NECTAR_PER_HARVEST, cell.nectarAmount, BEE_CARRY_CAPACITY - bee.carrying.nectar);
          cell.nectarAmount -= nectarHarvest;
          bee.carrying.nectar += nectarHarvest;

          const pollenHarvest = Math.min(POLLEN_PER_HARVEST, cell.pollenAmount, BEE_CARRY_CAPACITY - bee.carrying.pollen);
          cell.pollenAmount -= pollenHarvest;
          bee.carrying.pollen += pollenHarvest;
        }
        // Find nearest deposit target (waystation or hive)
        const target = findDepositTarget(bee, world);
        bee.state = BeeState.ReturningToHive;
        bee.path = computePath(bee.q, bee.r, target.q, target.r, world);
        world.pendingSounds.push('harvest');
      }
      break;
    }

    case BeeState.ReturningToHive: {
      if (bee.path.length === 0) {
        if (bee.carrying.nectar > 0 || bee.carrying.pollen > 0) {
          bee.state = BeeState.Depositing;
          bee.stateTimer = DEPOSIT_TICKS;
        } else {
          bee.state = BeeState.Idle;
        }
      }
      break;
    }

    case BeeState.Depositing: {
      bee.stateTimer--;
      if (bee.stateTimer <= 0) {
        const cell = world.grid.get(bee.q, bee.r);
        if (cell && cell.terrain === TerrainType.Waystation) {
          // Deposit at waystation
          if (bee.carrying.nectar > 0) {
            const space = WAYSTATION_NECTAR_CAPACITY - cell.nectarStored;
            const amount = Math.min(bee.carrying.nectar, space);
            cell.nectarStored += amount;
            bee.carrying.nectar -= amount;
          }
          if (bee.carrying.pollen > 0) {
            const space = WAYSTATION_POLLEN_CAPACITY - cell.pollenStored;
            const amount = Math.min(bee.carrying.pollen, space);
            cell.pollenStored += amount;
            bee.carrying.pollen -= amount;
          }
          // If still carrying (waystation full), continue to hive
          if (bee.carrying.nectar > 0.001 || bee.carrying.pollen > 0.001) {
            bee.state = BeeState.ReturningToHive;
            bee.path = computePath(bee.q, bee.r, 0, 0, world);
          } else {
            bee.state = BeeState.Idle;
          }
        } else {
          // Deposit at hive
          depositAtHive(bee, world);
          bee.state = BeeState.Idle;
        }
      }
      break;
    }
  }
}

function updateNurse(bee: BeeEntity, world: World): void {
  switch (bee.state) {
    case BeeState.Idle: {
      const broodCell = findBroodNeedingCare(world);
      if (broodCell) {
        bee.targetQ = broodCell.q;
        bee.targetR = broodCell.r;
        bee.state = BeeState.FlyingToBrood;
        bee.path = computePath(bee.q, bee.r, broodCell.q, broodCell.r, world);
        break;
      }
      const procCell = findProcessingCell(world);
      if (procCell) {
        bee.targetQ = procCell.q;
        bee.targetR = procCell.r;
        bee.state = BeeState.FlyingToProcessing;
        bee.path = computePath(bee.q, bee.r, procCell.q, procCell.r, world);
        break;
      }
      const emptyBrood = findEmptyBrood(world);
      if (emptyBrood && world.resources.honey >= BROOD_HONEY_COST && world.resources.beeBread >= BROOD_BEE_BREAD_COST) {
        if (world.deductHoney(BROOD_HONEY_COST) && world.deductBeeBread(BROOD_BEE_BREAD_COST)) {
          emptyBrood.broodActive = true;
          emptyBrood.broodProgress = 0;
        }
      }
      break;
    }

    case BeeState.FlyingToBrood: {
      if (bee.path.length === 0) {
        const cell = world.grid.get(bee.q, bee.r);
        if (cell && cell.terrain === TerrainType.Brood && cell.broodActive) {
          bee.state = BeeState.Tending;
          bee.stateTimer = TEND_TICKS;
        } else {
          bee.state = BeeState.Idle;
        }
      }
      break;
    }

    case BeeState.Tending: {
      bee.stateTimer--;
      if (bee.stateTimer <= 0) {
        const cell = world.grid.get(bee.q, bee.r);
        if (cell && cell.broodActive) {
          cell.broodProgress += BROOD_TEND_AMOUNT;
        }
        bee.state = BeeState.Idle;
      }
      break;
    }

    case BeeState.FlyingToProcessing: {
      if (bee.path.length === 0) {
        const cell = world.grid.get(bee.q, bee.r);
        if (cell && cell.terrain === TerrainType.Processing && cell.nectarStored > 0) {
          bee.state = BeeState.Processing;
          bee.stateTimer = PROCESS_TICKS;
        } else {
          bee.state = BeeState.Idle;
        }
      }
      break;
    }

    case BeeState.Processing: {
      bee.stateTimer--;
      if (bee.stateTimer <= 0) {
        const cell = world.grid.get(bee.q, bee.r);
        if (cell && cell.nectarStored > 0) {
          const processed = Math.min(PROCESSING_RATE * 5, cell.nectarStored);
          cell.nectarStored -= processed;
          const honeyProduced = processed * NECTAR_TO_HONEY_RATIO;

          const storageCells = world.grid.cellsOfType(TerrainType.HoneyStorage);
          for (const sc of storageCells) {
            const space = 5 - sc.honeyStored;
            if (space > 0) {
              const amount = Math.min(honeyProduced, space);
              sc.honeyStored += amount;
              world.resources.honey += amount;
              world.resources.nectar -= processed;
              break;
            }
          }
        }
        bee.state = BeeState.Idle;
      }
      break;
    }
  }
}

function updateScout(bee: BeeEntity, world: World): void {
  switch (bee.state) {
    case BeeState.Idle: {
      // Don't go exploring if energy is low and there's no food to recover with
      if (bee.energy < ENERGY_HUNGER_THRESHOLD * 1.5) {
        const hasFood = findHoneyCellWithHoney(world, bee.q, bee.r) !== null;
        if (!hasFood) {
          // Stay at hive to conserve energy (idle drain is much lower than flying)
          if (bee.q !== 0 || bee.r !== 0) {
            bee.path = computePath(bee.q, bee.r, 0, 0, world);
            bee.state = BeeState.ReturningToHive;
          }
          break;
        }
      }

      // Re-evaluate best base (hive or waystation)
      const base = findScoutBase(bee, world);
      bee.baseQ = base.q;
      bee.baseR = base.r;

      const target = findUnexploredTarget(bee, world);
      if (target) {
        bee.explorationTarget = target;
        bee.targetQ = target.q;
        bee.targetR = target.r;
        bee.state = BeeState.FlyingToExplore;
        bee.path = computePath(bee.q, bee.r, target.q, target.r, world);
      }
      break;
    }

    case BeeState.FlyingToExplore: {
      if (bee.path.length === 0) {
        bee.state = BeeState.Exploring;
        bee.stateTimer = 10;
      }
      break;
    }

    case BeeState.Exploring: {
      bee.stateTimer--;
      if (bee.stateTimer <= 0) {
        bee.state = BeeState.ReturningToHive;
        bee.path = computePath(bee.q, bee.r, bee.baseQ, bee.baseR, world);
      }
      break;
    }

    case BeeState.ReturningToHive: {
      if (bee.path.length === 0) {
        // Only waggle dance if we actually explored something
        if (bee.explorationTarget) {
          bee.state = BeeState.WaggleDancing;
          bee.danceTicks = WAGGLE_DANCE_TICKS;
        } else {
          bee.state = BeeState.Idle;
        }
      }
      break;
    }

    case BeeState.WaggleDancing: {
      bee.danceTicks--;
      if (bee.danceTicks <= 0) {
        bee.explorationTarget = null;
        bee.state = BeeState.Idle;
      }
      break;
    }
  }
}

function updateHauler(bee: BeeEntity, world: World): void {
  switch (bee.state) {
    case BeeState.Idle: {
      // Find waystation with resources to pick up
      const ws = findWaystationWithResources(world);
      if (ws) {
        bee.targetQ = ws.q;
        bee.targetR = ws.r;
        bee.state = BeeState.FlyingToWaystation;
        bee.path = computePath(bee.q, bee.r, ws.q, ws.r, world);
      }
      break;
    }

    case BeeState.FlyingToWaystation: {
      if (bee.path.length === 0) {
        const cell = world.grid.get(bee.q, bee.r);
        if (cell && cell.terrain === TerrainType.Waystation && waystationTotalResources(cell, world) > 0.01) {
          bee.state = BeeState.PickingUp;
          bee.stateTimer = HAULER_PICKUP_TICKS;
        } else {
          bee.state = BeeState.Idle;
        }
      }
      break;
    }

    case BeeState.PickingUp: {
      bee.stateTimer--;
      if (bee.stateTimer <= 0) {
        const cell = world.grid.get(bee.q, bee.r);
        if (cell) {
          let capacity = HAULER_CARRY_CAPACITY;

          // Pick up nectar from waystation
          const nectarPickup = Math.min(capacity, cell.nectarStored);
          cell.nectarStored -= nectarPickup;
          bee.carrying.nectar += nectarPickup;
          capacity -= nectarPickup;

          // Pick up pollen from waystation
          if (capacity > 0) {
            const pollenPickup = Math.min(capacity, cell.pollenStored);
            cell.pollenStored -= pollenPickup;
            bee.carrying.pollen += pollenPickup;
            capacity -= pollenPickup;
          }

          // Draw remaining capacity from adjacent storage
          if (capacity > 0) {
            for (const nb of hexNeighbors(cell.q, cell.r)) {
              if (capacity <= 0) break;
              const adj = world.grid.get(nb.q, nb.r);
              if (!adj) continue;
              if (adj.terrain === TerrainType.NectarStorage && adj.nectarStored > 0) {
                const amount = Math.min(capacity, adj.nectarStored);
                adj.nectarStored -= amount;
                bee.carrying.nectar += amount;
                capacity -= amount;
              } else if (adj.terrain === TerrainType.PollenStorage && adj.pollenStored > 0) {
                const amount = Math.min(capacity, adj.pollenStored);
                adj.pollenStored -= amount;
                bee.carrying.pollen += amount;
                capacity -= amount;
              }
            }
          }
        }
        bee.state = BeeState.ReturningToHive;
        bee.path = computePath(bee.q, bee.r, 0, 0, world);
      }
      break;
    }

    case BeeState.ReturningToHive: {
      if (bee.path.length === 0) {
        if (bee.carrying.nectar > 0 || bee.carrying.pollen > 0) {
          bee.state = BeeState.Depositing;
          bee.stateTimer = DEPOSIT_TICKS;
        } else {
          bee.state = BeeState.Idle;
        }
      }
      break;
    }

    case BeeState.Depositing: {
      bee.stateTimer--;
      if (bee.stateTimer <= 0) {
        depositAtHive(bee, world);
        bee.state = BeeState.Idle;
      }
      break;
    }
  }
}

function updateBuilder(bee: BeeEntity, world: World): void {
  if (bee.state === BeeState.Idle) {
    bee.state = BeeState.IdleAtHive;
  }

  if (bee.state === BeeState.IdleAtHive) {
    // Look for wax works cells with honey to work at
    const waxWorks = findWaxWorksWithHoney(world, bee.q, bee.r);
    if (waxWorks) {
      bee.targetQ = waxWorks.q;
      bee.targetR = waxWorks.r;
      bee.state = BeeState.FlyingToWaxWorks;
      bee.path = computePath(bee.q, bee.r, waxWorks.q, waxWorks.r, world);
      return;
    }
    // No wax works available — head home if not already there
    if (bee.q !== 0 || bee.r !== 0) {
      bee.path = computePath(bee.q, bee.r, 0, 0, world);
      bee.state = BeeState.ReturningToHive;
    }
    return;
  }

  if (bee.state === BeeState.FlyingToWaxWorks) {
    if (bee.path.length === 0) {
      const cell = world.grid.get(bee.q, bee.r);
      if (cell && cell.terrain === TerrainType.WaxWorks && cell.honeyStored > 0.01) {
        bee.state = BeeState.ProducingWax;
        bee.stateTimer = BUILDER_WAX_WORK_TICKS;
      } else {
        bee.state = BeeState.IdleAtHive;
      }
    }
    return;
  }

  if (bee.state === BeeState.ProducingWax) {
    bee.stateTimer--;
    const cell = world.grid.get(bee.q, bee.r);
    if (cell && cell.honeyStored > 0) {
      const consumed = Math.min(WAX_WORKS_BUILDER_RATE, cell.honeyStored);
      cell.honeyStored -= consumed;
      world.resources.wax += consumed * WAX_WORKS_HONEY_TO_WAX_RATIO;
    }
    if (bee.stateTimer <= 0 || !cell || cell.honeyStored <= 0.01) {
      bee.state = BeeState.IdleAtHive;
    }
    return;
  }

  if (bee.state === BeeState.ReturningToHive && bee.path.length === 0) {
    bee.state = BeeState.IdleAtHive;
  }
}

function findWaxWorksWithHoney(world: World, beeQ: number, beeR: number): HexCell | null {
  const cells = world.grid.cellsOfType(TerrainType.WaxWorks);
  let best: HexCell | null = null;
  let bestDist = Infinity;
  for (const c of cells) {
    if (c.honeyStored <= 0.01) continue;
    const dist = hexDistance(beeQ, beeR, c.q, c.r);
    if (dist < bestDist) {
      bestDist = dist;
      best = c;
    }
  }
  return best;
}

function findHoneyCellWithHoney(world: World, beeQ?: number, beeR?: number): HexCell | null {
  const cells = world.grid.cellsOfType(TerrainType.HoneyStorage);
  let best: HexCell | null = null;
  let bestDist = Infinity;
  for (const c of cells) {
    if (c.honeyStored <= 0.01) continue;
    if (beeQ !== undefined && beeR !== undefined) {
      const dist = hexDistance(beeQ, beeR, c.q, c.r);
      if (dist < bestDist) {
        bestDist = dist;
        best = c;
      }
    } else {
      // Fallback: pick the one with the most honey
      if (c.honeyStored > (best?.honeyStored ?? 0)) {
        best = c;
      }
    }
  }
  return best;
}

/** Total nectar+pollen in a waystation plus its adjacent storage cells */
function waystationTotalResources(ws: HexCell, world: World): number {
  let total = ws.nectarStored + ws.pollenStored;
  for (const nb of hexNeighbors(ws.q, ws.r)) {
    const cell = world.grid.get(nb.q, nb.r);
    if (!cell) continue;
    if (cell.terrain === TerrainType.NectarStorage) total += cell.nectarStored;
    if (cell.terrain === TerrainType.PollenStorage) total += cell.pollenStored;
  }
  return total;
}

function findWaystationWithResources(world: World): HexCell | null {
  const waystations = world.grid.waystationCells();
  let best: HexCell | null = null;
  let bestResources = 0;
  for (const ws of waystations) {
    const total = waystationTotalResources(ws, world);
    if (total > 0.01 && total > bestResources) {
      bestResources = total;
      best = ws;
    }
  }
  return best;
}

function findBestFlower(bee: BeeEntity, world: World): HexCell | null {
  const flowers = world.grid.cellsOfType(TerrainType.Flower);
  const waystations = world.grid.cellsOfType(TerrainType.Waystation);
  let best: HexCell | null = null;
  let bestScore = -Infinity;

  for (const f of flowers) {
    if (!f.explored) continue;
    if (f.nectarAmount < 0.01 && f.pollenAmount < 0.01) continue;
    const nectarRatio = f.nectarMax > 0 ? f.nectarAmount / f.nectarMax : 0;
    const pollenRatio = f.pollenMax > 0 ? f.pollenAmount / f.pollenMax : 0;
    if (nectarRatio < 0.3 && pollenRatio < 0.3) continue;
    const dist = hexDistance(bee.q, bee.r, f.q, f.r);

    // Bonus for flowers near waystations (closer = higher bonus, 0-1 range)
    let wsProximity = 0;
    for (const ws of waystations) {
      const wsDist = hexDistance(f.q, f.r, ws.q, ws.r);
      if (wsDist < SCORE_WAYSTATION_MAX_DIST) {
        const proximity = 1 - wsDist / SCORE_WAYSTATION_MAX_DIST;
        if (proximity > wsProximity) wsProximity = proximity;
      }
    }

    const score = SCORE_DISTANCE_WEIGHT * dist
      + SCORE_NECTAR_WEIGHT * nectarRatio
      + SCORE_POLLEN_WEIGHT * pollenRatio
      + SCORE_WAYSTATION_WEIGHT * wsProximity
      + SCORE_JITTER * (Math.random() - 0.5);

    if (score > bestScore) {
      bestScore = score;
      best = f;
    }
  }
  return best;
}

function findBroodNeedingCare(world: World): HexCell | null {
  const broods = world.grid.cellsOfType(TerrainType.Brood);
  for (const b of broods) {
    if (b.broodActive && b.broodProgress < 1) return b;
  }
  return null;
}

function findProcessingCell(world: World): HexCell | null {
  const cells = world.grid.cellsOfType(TerrainType.Processing);
  for (const c of cells) {
    if (c.nectarStored > 0.01) return c;
  }
  return null;
}

function findEmptyBrood(world: World): HexCell | null {
  const broods = world.grid.cellsOfType(TerrainType.Brood);
  for (const b of broods) {
    if (!b.broodActive) return b;
  }
  return null;
}

function findScoutBase(bee: BeeEntity, world: World): { q: number; r: number } {
  const candidates: { q: number; r: number; range: number }[] = [
    { q: 0, r: 0, range: SCOUT_EXPLORE_RANGE },
  ];

  const waystations = world.grid.waystationCells();
  for (const ws of waystations) {
    candidates.push({ q: ws.q, r: ws.r, range: SCOUT_WAYSTATION_EXPLORE_RANGE });
  }

  let bestBase = { q: 0, r: 0 };
  let bestScore = -Infinity;

  for (const cand of candidates) {
    // Count unexplored frontier hexes within range of this base
    let frontier = 0;
    for (const cell of world.grid.cells.values()) {
      if (cell.explored) continue;
      if (hexDistance(cand.q, cand.r, cell.q, cell.r) > cand.range) continue;
      const neighbors = hexNeighbors(cell.q, cell.r);
      for (const n of neighbors) {
        const nc = world.grid.get(n.q, n.r);
        if (nc && nc.explored) { frontier++; break; }
      }
    }
    const travelDist = hexDistance(bee.q, bee.r, cand.q, cand.r);
    const score = frontier - travelDist * 0.5;
    if (score > bestScore) {
      bestScore = score;
      bestBase = { q: cand.q, r: cand.r };
    }
  }

  return bestBase;
}

function findUnexploredTarget(bee: BeeEntity, world: World): { q: number; r: number } | null {
  let best: { q: number; r: number } | null = null;
  let bestScore = -Infinity;

  const isAtHive = bee.baseQ === 0 && bee.baseR === 0;
  const range = isAtHive ? SCOUT_EXPLORE_RANGE : SCOUT_WAYSTATION_EXPLORE_RANGE;

  // Ensure chunks exist around the base so there's frontier to find
  ensureChunksAroundPoint(world, bee.baseQ, bee.baseR, 1);

  for (const cell of world.grid.cells.values()) {
    if (cell.explored) continue;
    const distFromBase = hexDistance(bee.baseQ, bee.baseR, cell.q, cell.r);
    if (distFromBase > range) continue;

    const neighbors = hexNeighbors(cell.q, cell.r);
    let nearExplored = false;
    for (const n of neighbors) {
      const nc = world.grid.get(n.q, n.r);
      if (nc && nc.explored) { nearExplored = true; break; }
    }
    if (!nearExplored) continue;

    const beeDist = hexDistance(bee.q, bee.r, cell.q, cell.r);
    const score = -beeDist + Math.random() * 5;

    if (score > bestScore) {
      bestScore = score;
      best = { q: cell.q, r: cell.r };
    }
  }
  return best;
}

/** Greedy walk: pick neighbor closest to target each step */
export function computePath(fromQ: number, fromR: number, toQ: number, toR: number, _world: World): { q: number; r: number }[] {
  const path: { q: number; r: number }[] = [];
  let cq = fromQ;
  let cr = fromR;

  const maxSteps = hexDistance(fromQ, fromR, toQ, toR) + 5;
  for (let i = 0; i < maxSteps; i++) {
    if (cq === toQ && cr === toR) break;

    const neighbors = hexNeighbors(cq, cr);
    let bestDist = Infinity;
    let bestN = neighbors[0];

    for (const n of neighbors) {
      const d = hexDistance(n.q, n.r, toQ, toR);
      if (d < bestDist) {
        bestDist = d;
        bestN = n;
      }
    }

    path.push(bestN);
    cq = bestN.q;
    cr = bestN.r;
  }

  return path;
}
